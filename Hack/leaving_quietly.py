...
def leaving_quietly():
    ...
    arp_response.pdst = "192.168.136.2"  # Router's IP address
    
    # Set the MAC address of the router as the target hardware address
    arp_response.hwdst = "00:50:56:fb:a3:aa"  # Router's MAC address
    
    # Set the real MAC address of the victim (so the router learns the correct binding)
    arp_response.hwsrc = "00:0c:29:f1:2c:5b"  # Victim's MAC address
    
    # Set the victim's IP address as the source IP
    arp_response.psrc = "192.168.136.130"  # Victim's IP address
    ...
    arp_response.pdst = "192.168.1.129"  # Victim's IP address
    
    # Set the victim's MAC address as the destination hardware
    arp_response.hwdst = "00:0c:29:6b:b0:e3"  # Victim's MAC address
    
    # Set the real MAC address of the router (to reestablish trust)
    arp_response.hwsrc = "00:50:56:ef:bc:68"  # Router's MAC address
